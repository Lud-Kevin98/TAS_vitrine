...
function LangDropdown({ block = false }: { block?: boolean }) {
  ...
  return (
    <div ref={ref} className={`relative ${block ? 'w-full' : ''}`}>
      <button
        onClick={() => setOpen((v) => !v)}
        className={`inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-600 transition-colors hover:text-navy-900 ${block ? 'w-full justify-center' : ''}`}
      ...
      {open && (
        <ul className={`absolute right-0 z-50 mt-2 min-w-[9rem] overflow-hidden rounded-xl border border-slate-200 bg-white p-1 shadow-medium ${block ? 'left-0' : ''}`} role="listbox">
          {LANGS.map((l) => (
            <li key={l.code}>
              <button
    ...
    <header className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'border-b border-slate-200/70 bg-white/85 backdrop-blur-xl' : 'bg-white/70 backdrop-blur-sm'}`}>
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-5 sm:px-8">
        ...
        <div className="flex shrink-0 items-center gap-2.5">
          <div className="hidden xl:block"><LangDropdown /></div>
      ...
      {open && (
        <div className="fixed inset-x-0 top-16 bottom-0 z-40 overflow-y-auto border-t border-slate-200 bg-white xl:hidden animate-fade-in">
          <ul className="flex flex-col gap-1 p-5">
            ...
            <li className="mt-3"><LangDropdown block /></li>
            <li className="mt-2">
