...
export type Content = {
  ...
  services: { eyebrow: string; title: string; sub: string; offer: { title: string; desc: string }[]; methodE: string; methodT: string; methodS: string; method: { title: string; desc: string }[]; ctaT: string; ctaS: string }
  ...
  footer: { tagline: string; cols: { title: string; links: { label: string; to: string }[] }[]; hosted: string; legal1: string; legal2: string; legal3: string; legal4: string; rights: string }
  notFound: { code: string; title: string; sub: string; home: string; contact: string }
  ctaDefault: { title: string; sub: string }
}

const NAV_TO = ['/', '/a-propos', '/produits', '/solutions', '/services', '/tarifs', '/ressources', '/carrieres', '/contact']

const TR: Record<Lang, Content> = {
  /* ═══════════════════════ FR ═══════════════════════ */
  fr: {
    nav: ['Accueil', 'À propos', 'Produits', 'Solutions', 'Services', 'Tarifs', 'Ressources', 'Carrières', 'Contact'].map((label, i) => ({ label, to: NAV_TO[i] })),
    ...
    services: {
      eyebrow: 'Services & conseil',
      title: 'De l’audit à l’automatisation, nous digitalisons vos opérations.',
      sub: 'TAS n’est pas qu’un logiciel. Nous vous accompagnons de bout en bout : comprendre vos processus, les optimiser, les mettre en œuvre sur la plateforme, former vos équipes et assurer le support.',
      offer: [
        { title: 'Audit & diagnostic', desc: 'Analyse de vos processus sur le terrain, cartographie des flux et identification des priorités de digitalisation.' },
        { title: 'Conseil en digitalisation', desc: 'Stratégie, feuille de route et choix des outils — des recommandations concrètes, adaptées à votre réalité et à votre budget.' },
        { title: 'Mise en œuvre sur la plateforme', desc: 'Configuration de la plateforme TAS sur vos processus réels : activation des modules, paramétrage des rôles et des règles.' },
        { title: 'Développement sur mesure', desc: 'Modules spécifiques, intégrations et API pour connecter TAS à vos outils existants (comptabilité, e-commerce, mobile money…).' },
        { title: 'Formation & conduite du changement', desc: 'Formation de vos équipes, documentation et accompagnement de l’adoption — la digitalisation ne réussit que si elle est utilisée.' },
        { title: 'Support & maintenance', desc: 'Support réactif, supervision, évolutions continues et sécurité. Nous restons votre partenaire dans la durée.' },
      ],
      methodE: 'Notre méthode',
      methodT: 'Une démarche claire, du diagnostic au résultat.',
      methodS: 'Nous avançons par étapes courtes et concrètes, avec vous à chaque décision.',
      method: [
        { title: 'Comprendre', desc: 'Nous écoutons le terrain et cartographions vos processus et vos irritants.' },
        { title: 'Concevoir', desc: 'Nous définissons la cible : processus optimisés, modules, priorités.' },
        { title: 'Mettre en œuvre', desc: 'Nous configurons la plateforme et développons ce qui doit l’être.' },
        { title: 'Déployer', desc: 'Nous formons vos équipes et lançons progressivement, sans casser l’activité.' },
        { title: 'Faire évoluer', desc: 'Nous mesurons, ajustons et ajoutons des modules au rythme de vos besoins.' },
      ],
      ctaT: 'Parlons de votre projet de digitalisation.',
      ctaS: 'Un premier échange gratuit pour comprendre vos enjeux et vous proposer une démarche concrète.',
    },
  ...
  /* ═══════════════════════ EN ═══════════════════════ */
  en: {
    nav: ['Home', 'About', 'Products', 'Solutions', 'Services', 'Pricing', 'Resources', 'Careers', 'Contact'].map((label, i) => ({ label, to: NAV_TO[i] })),
    ...
    services: {
      ...
      title: 'From audit to automation, we digitalise your operations.',
      sub: 'TAS is more than software. We support you end to end: understand your processes, optimise them, implement them on the platform, train your teams and provide support.',
      offer: [
        { title: 'Audit & diagnosis', desc: 'On-the-ground analysis of your processes, flow mapping and identification of digitalisation priorities.' },
        { title: 'Digitalisation consulting', desc: 'Strategy, roadmap and tool selection — concrete recommendations tailored to your reality and budget.' },
        { title: 'Platform implementation', desc: 'Configuring the TAS platform on your real processes: activating modules, setting up roles and rules.' },
        { title: 'Custom development', desc: 'Specific modules, integrations and APIs to connect TAS to your existing tools (accounting, e-commerce, mobile money…).' },
        { title: 'Training & change management', desc: 'Team training, documentation and adoption support — digitalisation only succeeds when it is actually used.' },
        { title: 'Support & maintenance', desc: 'Responsive support, monitoring, continuous improvements and security. We remain your long-term partner.' },
      ],
      methodE: 'Our method',
      methodT: 'A clear approach, from diagnosis to results.',
      methodS: 'We move in short, concrete steps, with you at every decision.',
      method: [
        { title: 'Understand', desc: 'We listen to the field and map your processes and pain points.' },
        { title: 'Design', desc: 'We define the target: optimised processes, modules, priorities.' },
        { title: 'Implement', desc: 'We configure the platform and build whatever needs building.' },
        { title: 'Deploy', desc: 'We train your teams and roll out gradually, without disrupting operations.' },
        { title: 'Evolve', desc: 'We measure, adjust and add modules at the pace of your needs.' },
      ],
      ctaT: 'Let’s talk about your digitalisation project.',
      ctaS: 'A free first conversation to understand your challenges and propose a concrete approach.',
    },
    ...
    footer: { tagline: 'The modular, configurable platform that adapts to every company’s processes.', cols: [{ title: 'Products', links: [{ label: 'TAS Platform', to: '/produits' }, { label: 'TAS Logistics', to: '/produits#logistics' }, { label: 'TAS People', to: '/produits#people' }, { label: 'Finance & CRM', to: '/produits#crm' }] }, { title: 'Solutions', links: [{ label: 'Transport', to: '/solutions#transport' }, { label: 'Retail', to: '/solutions#commerce' }, { label: 'Health', to: '/solutions#sante' }, { label: 'Education', to: '/solutions#education' }] }, { title: 'Company', links: [{ label: 'About', to: '/a-propos' }, { label: 'Careers', to: '/carrieres' }, { label: 'Pricing', to: '/tarifs' }, { label: 'Contact', to: '/contact' }] }, { title: 'Resources', links: [{ label: 'Documentation', to: '/ressources#documentation' }, { label: 'FAQ', to: '/ressources#faq' }, { label: 'Blog', to: '/ressources#blog' }, { label: 'Help center', to: '/ressources#aide' }] }], hosted: 'Hosted in Germany 🇩🇪', legal1: 'Legal notice', legal2: 'Privacy', legal3: 'Cookies', legal4: 'Terms', rights: 'All rights reserved.' },
  ...
  /* ═══════════════════════ DE ═══════════════════════ */
  de: {
    nav: ['Start', 'Über uns', 'Produkte', 'Lösungen', 'Leistungen', 'Preise', 'Ressourcen', 'Karriere', 'Kontakt'].map((label, i) => ({ label, to: NAV_TO[i] })),
    ...
    services: {
      eyebrow: 'Leistungen & Beratung',
      title: 'Vom Audit bis zur Automatisierung — wir digitalisieren Ihre Abläufe.',
      sub: 'TAS ist mehr als Software. Wir begleiten Sie durchgängig: Prozesse verstehen, optimieren, auf der Plattform umsetzen, Teams schulen und Support leisten.',
      offer: [
        { title: 'Audit & Diagnose', desc: 'Analyse Ihrer Prozesse vor Ort, Ablaufkartierung und Ermittlung der Digitalisierungsprioritäten.' },
        { title: 'Digitalisierungsberatung', desc: 'Strategie, Roadmap und Tool-Auswahl — konkrete Empfehlungen, passend zu Ihrer Realität und Ihrem Budget.' },
        { title: 'Umsetzung auf der Plattform', desc: 'Konfiguration der TAS-Plattform auf Ihren realen Prozessen: Module aktivieren, Rollen und Regeln einrichten.' },
        { title: 'Individualentwicklung', desc: 'Spezifische Module, Integrationen und APIs, um TAS mit Ihren bestehenden Tools zu verbinden (Buchhaltung, E-Commerce, Mobile Money…).' },
        { title: 'Schulung & Change-Management', desc: 'Schulung Ihrer Teams, Dokumentation und Adoptionsbegleitung — Digitalisierung gelingt nur, wenn sie genutzt wird.' },
        { title: 'Support & Wartung', desc: 'Reaktionsschneller Support, Überwachung, laufende Weiterentwicklung und Sicherheit. Wir bleiben Ihr langfristiger Partner.' },
      ],
      methodE: 'Unsere Methode',
      methodT: 'Ein klares Vorgehen — von der Diagnose zum Ergebnis.',
      methodS: 'Wir gehen in kurzen, konkreten Schritten vor — mit Ihnen bei jeder Entscheidung.',
      method: [
        { title: 'Verstehen', desc: 'Wir hören dem Feld zu und kartieren Ihre Prozesse und Schmerzpunkte.' },
        { title: 'Konzipieren', desc: 'Wir definieren das Ziel: optimierte Prozesse, Module, Prioritäten.' },
        { title: 'Umsetzen', desc: 'Wir konfigurieren die Plattform und entwickeln, was nötig ist.' },
        { title: 'Einführen', desc: 'Wir schulen Ihre Teams und führen schrittweise ein, ohne den Betrieb zu stören.' },
        { title: 'Weiterentwickeln', desc: 'Wir messen, justieren und ergänzen Module im Takt Ihres Bedarfs.' },
      ],
      ctaT: 'Sprechen wir über Ihr Digitalisierungsprojekt.',
      ctaS: 'Ein kostenloses Erstgespräch, um Ihre Herausforderungen zu verstehen und ein konkretes Vorgehen vorzuschlagen.',
    },
