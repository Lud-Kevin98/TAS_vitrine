...
const contact = {
  ...
  phone: '+237 6 57 08 69 84',        // appel + WhatsApp (Cameroun)
  whatsapp: '+237 6 57 08 69 84',
  whatsapp2: '+49 176 29434276',      // WhatsApp uniquement (Allemagne)
...
function InfoCard({ icon: Icon, title, desc }: CardItem) {
  return (
    <div className="group relative overflow-hidden bg-white rounded-2xl p-6 border border-slate-100 hover:border-blue-200 shadow-card hover:shadow-strong hover:-translate-y-1.5 transition-all duration-300">
      <div className="absolute top-0 left-0 h-1 w-0 group-hover:w-full bg-gradient-to-r from-blue-500 via-cyan-400 to-blue-500 transition-all duration-500" />
      <div className="absolute -right-8 -top-8 w-24 h-24 rounded-full bg-blue-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      <div className="relative w-12 h-12 rounded-2xl flex items-center justify-center mb-4 bg-brand shadow-glow group-hover:scale-110 group-hover:rotate-3 transition-transform"><Icon size={20} className="text-white" /></div>
      <h3 className="relative font-display font-bold text-navy-900 text-lg mb-2 group-hover:text-blue-700 transition-colors">{title}</h3>
      <p className="relative font-body text-slate-500 text-sm leading-relaxed">{desc}</p>
    </div>
  )
}
...
function HowSteps({ lang }: { lang: Lang }) {
  const t = content[lang]
  return <section id="comment" className="py-24 bg-gradient-to-br from-slate-50 via-blue-50/50 to-white"><div className="max-w-7xl mx-auto px-4 sm:px-6"><div className="reveal text-center max-w-3xl mx-auto mb-14 space-y-4"><Label>{t.howLabel}</Label><h2 className="font-display font-bold text-3xl sm:text-4xl lg:text-[42px] text-slate-900">{t.howTitle}</h2></div><div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">{t.howSteps.map(([title, desc]: string[], i: number) => <div key={title} className="group relative bg-white rounded-2xl border border-slate-100 hover:border-blue-200 p-6 shadow-soft hover:shadow-medium hover:-translate-y-1 transition-all"><div className="w-12 h-12 bg-gradient-to-br from-blue-600 to-cyan-500 rounded-2xl flex items-center justify-center text-white font-display font-bold mb-5 shadow-glow group-hover:scale-105 transition-transform">{String(i+1).padStart(2,'0')}</div><h3 className="font-display font-bold text-slate-900 text-lg mb-2">{title}</h3><p className="font-body text-slate-500 text-sm leading-relaxed">{desc}</p></div>)}</div></div></section>
}
